import requests
import base64
import time
from typing import Any, Dict, Optional
from datetime import datetime, timedelta
import json
from decimal import Decimal

from requests.exceptions import HTTPError

from cryptography.hazmat.primitives import hashes
from cryptography.hazmat.primitives.asymmetric import padding, rsa
from cryptography.exceptions import InvalidSignature

import websockets

from utils.utils import KalshiEnvironment


class KalshiBaseClient:
    """Base client class for interacting with the Kalshi API."""

    def __init__(
        self,
        key_id: str,
        private_key: rsa.RSAPrivateKey,
        environment: KalshiEnvironment = KalshiEnvironment.DEMO,
    ):
        self.key_id = key_id
        self.private_key = private_key
        self.environment = environment
        self.last_api_call = datetime.now()

        if self.environment == KalshiEnvironment.DEMO:
            self.HTTP_BASE_URL = "https://demo-api.kalshi.co"
            self.WS_BASE_URL = "wss://demo-api.kalshi.co"
        elif self.environment == KalshiEnvironment.PROD:
            self.HTTP_BASE_URL = "https://api.elections.kalshi.com"
            self.WS_BASE_URL = "wss://api.elections.kalshi.com"
        else:
            raise ValueError("Invalid environment")

    def request_headers(self, method: str, path: str) -> Dict[str, Any]:
        current_time_milliseconds = int(time.time() * 1000)
        timestamp_str = str(current_time_milliseconds)

        path_parts = path.split("?")
        msg_string = timestamp_str + method + path_parts[0]
        signature = self.sign_pss_text(msg_string)

        return {
            "Content-Type": "application/json",
            "KALSHI-ACCESS-KEY": self.key_id,
            "KALSHI-ACCESS-SIGNATURE": signature,
            "KALSHI-ACCESS-TIMESTAMP": timestamp_str,
        }

    def sign_pss_text(self, text: str) -> str:
        message = text.encode("utf-8")
        try:
            signature = self.private_key.sign(
                message,
                padding.PSS(
                    mgf=padding.MGF1(hashes.SHA256()),
                    salt_length=padding.PSS.DIGEST_LENGTH,
                ),
                hashes.SHA256(),
            )
            return base64.b64encode(signature).decode("utf-8")
        except InvalidSignature as e:
            raise ValueError("RSA sign PSS failed") from e


class KalshiHttpClient(KalshiBaseClient):
    """Client for handling HTTP connections to the Kalshi API."""

    def __init__(
        self,
        key_id: str,
        private_key: rsa.RSAPrivateKey,
        environment: KalshiEnvironment = KalshiEnvironment.DEMO,
    ):
        super().__init__(key_id, private_key, environment)
        self.host = self.HTTP_BASE_URL
        self.base_trade_url = "/trade-api/v2"
        self.exchange_url = f"{self.base_trade_url}/exchange"
        self.markets_url = f"{self.base_trade_url}/markets"
        self.portfolio_url = f"{self.base_trade_url}/portfolio"
        self.account_url = f"{self.base_trade_url}/account"

    def rate_limit(self) -> None:
        threshold_ms = 100
        now = datetime.now()
        threshold_us = 1000 * threshold_ms
        threshold_s = threshold_ms / 1000
        if now - self.last_api_call < timedelta(microseconds=threshold_us):
            time.sleep(threshold_s)
        self.last_api_call = datetime.now()

    def raise_if_bad_response(self, response: requests.Response) -> None:
        if response.status_code not in range(200, 299):
            try:
                detail = response.text
            except Exception:
                detail = "<no response body>"
            raise HTTPError(
                f"{response.status_code} {response.reason}: {detail}",
                response=response,
            )

    def post(self, path: str, body: dict) -> Any:
        self.rate_limit()
        response = requests.post(
            self.host + path,
            json=body,
            headers=self.request_headers("POST", path),
        )
        self.raise_if_bad_response(response)
        return response.json()

    def get(self, path: str, params: Dict[str, Any] = {}) -> Any:
        self.rate_limit()
        response = requests.get(
            self.host + path,
            headers=self.request_headers("GET", path),
            params=params,
        )
        self.raise_if_bad_response(response)
        return response.json()

    def delete(self, path: str, params: Dict[str, Any] = {}) -> Any:
        self.rate_limit()
        response = requests.delete(
            self.host + path,
            headers=self.request_headers("DELETE", path),
            params=params,
        )
        self.raise_if_bad_response(response)
        return response.json()

    def get_balance(self) -> Dict[str, Any]:
        return self.get(self.portfolio_url + "/balance")

    def get_exchange_status(self) -> Dict[str, Any]:
        return self.get(self.exchange_url + "/status")

    def get_account_limits(self) -> Dict[str, Any]:
        return self.get(self.account_url + "/limits")

    def get_trades(
        self,
        ticker: Optional[str] = None,
        limit: Optional[int] = None,
        cursor: Optional[str] = None,
        max_ts: Optional[int] = None,
        min_ts: Optional[int] = None,
    ) -> Dict[str, Any]:
        params = {
            "ticker": ticker,
            "limit": limit,
            "cursor": cursor,
            "max_ts": max_ts,
            "min_ts": min_ts,
        }
        params = {k: v for k, v in params.items() if v is not None}
        return self.get(self.markets_url + "/trades", params=params)

    def get_market_trades(
        self,
        ticker: str,
        limit: int = 1000,
        cursor: Optional[str] = None,
        max_ts: Optional[int] = None,
        min_ts: Optional[int] = None,
    ):
        params = {
            "ticker": ticker,
            "limit": limit,
            "cursor": cursor,
            "max_ts": max_ts,
            "min_ts": min_ts,
        }
        params = {k: v for k, v in params.items() if v is not None}
        return self.get(self.markets_url + "/trades", params=params)

    def get_all_trades(
        self,
        ticker: str,
        max_ts: Optional[int] = None,
    ):
        all_trades = []
        cursor = None

        while True:
            resp = self.get_market_trades(
                ticker=ticker,
                cursor=cursor,
                max_ts=max_ts,
            )

            trades = resp.get("trades", [])
            all_trades.extend(trades)

            cursor = resp.get("cursor")
            if not cursor:
                break

        return all_trades

    def get_last_price_before_ts(
        self,
        ticker: str,
        ts: int,
    ):
        trades = self.get_all_trades(ticker, max_ts=ts)

        if not trades:
            return None, None

        last_trade = trades[0]
        price = last_trade.get("price")
        if price is None:
            return None, None

        price = float(price)
        return price, (1.0 - price)

    def get_market_prices(self, ticker: str):
        data = self.get(f"{self.markets_url}/{ticker}")
        m = data.get("market", {})

        def f(x):
            return float(x) if x is not None else None

        return {
            "ticker": m.get("ticker"),
            "yes_bid": f(m.get("yes_bid_dollars") or m.get("yes_bid")),
            "yes_ask": f(m.get("yes_ask_dollars") or m.get("yes_ask")),
            "no_bid": f(m.get("no_bid_dollars") or m.get("no_bid")),
            "no_ask": f(m.get("no_ask_dollars") or m.get("no_ask")),
            "last_price": f(m.get("price_dollars") or m.get("last_price")),
            "volume_24h": m.get("volume_24h"),
            "open_interest": m.get("open_interest"),
            "status": m.get("status"),
        }

    def get_orderbook(self, ticker: str, depth: int = None):
        """
        Kalshi returns bids only:
          - orderbook_fp.yes_dollars
          - orderbook_fp.no_dollars
        Each level is [price_dollars, count_fp], both strings.
        Best bid is the last element.
        Implied ask on one side = 1 - best bid on the opposite side.
        """
        params = {}
        if depth is not None:
            params["depth"] = depth

        data = self.get(f"{self.markets_url}/{ticker}/orderbook", params=params)
        ob = data.get("orderbook_fp", {})

        yes_levels_raw = ob.get("yes_dollars", []) or []
        no_levels_raw = ob.get("no_dollars", []) or []

        yes_levels = [(float(price), float(size)) for price, size in yes_levels_raw]
        no_levels = [(float(price), float(size)) for price, size in no_levels_raw]

        best_yes_bid = yes_levels[-1][0] if yes_levels else None
        best_no_bid = no_levels[-1][0] if no_levels else None

        best_yes_ask = (
            float(Decimal("1.0") - Decimal(str(best_no_bid)))
            if best_no_bid is not None
            else None
        )
        best_no_ask = (
            float(Decimal("1.0") - Decimal(str(best_yes_bid)))
            if best_yes_bid is not None
            else None
        )

        return {
            "yes_levels": yes_levels,
            "no_levels": no_levels,
            "best_yes_bid": best_yes_bid,
            "best_no_bid": best_no_bid,
            "best_yes_ask": best_yes_ask,
            "best_no_ask": best_no_ask,
        }

    def create_order(self, **body):
        return self.post("/trade-api/v2/portfolio/orders", body)

    def buy_contract(
        self,
        ticker: str,
        side: str,
        count: int,
        limit_price_dollars: float,
        client_order_id: Optional[str] = None,
        time_in_force: str = "immediate_or_cancel",
        post_only: bool = False,
        reduce_only: bool = False,
    ):
        body = {
            "ticker": ticker,
            "side": side.lower(),
            "action": "buy",
            "count": count,
            "client_order_id": client_order_id,
            "time_in_force": time_in_force,
            "post_only": post_only,
            "reduce_only": reduce_only,
        }

        if side.lower() == "yes":
            body["yes_price_dollars"] = f"{limit_price_dollars:.4f}"
        else:
            body["no_price_dollars"] = f"{limit_price_dollars:.4f}"

        body = {k: v for k, v in body.items() if v is not None}
        return self.create_order(**body)

    def sell_contract(
        self,
        ticker: str,
        side: str,
        count: int,
        limit_price_dollars: float,
        client_order_id: Optional[str] = None,
        time_in_force: str = "immediate_or_cancel",
        post_only: bool = False,
        reduce_only: bool = True,
    ):
        body = {
            "ticker": ticker,
            "side": side.lower(),
            "action": "sell",
            "count": count,
            "client_order_id": client_order_id,
            "time_in_force": time_in_force,
            "post_only": post_only,
            "reduce_only": reduce_only,
        }

        if side.lower() == "yes":
            body["yes_price_dollars"] = f"{limit_price_dollars:.4f}"
        else:
            body["no_price_dollars"] = f"{limit_price_dollars:.4f}"

        body = {k: v for k, v in body.items() if v is not None}
        return self.create_order(**body)


class KalshiWebSocketClient(KalshiBaseClient):
    """Client for handling WebSocket connections to the Kalshi API."""

    def __init__(
        self,
        key_id: str,
        private_key: rsa.RSAPrivateKey,
        environment: KalshiEnvironment = KalshiEnvironment.PROD,
    ):
        super().__init__(key_id, private_key, environment)
        self.ws = None
        self.url_suffix = "/trade-api/ws/v2"
        self.message_id = 1
        self._tickers: list[str] = []
        self._message_handler = None

    async def connect(self):
        host = self.WS_BASE_URL + self.url_suffix
        auth_headers = self.request_headers("GET", self.url_suffix)
        async with websockets.connect(host, additional_headers=auth_headers) as websocket:
            self.ws = websocket
            await self.on_open()
            await self.handler()

    async def on_open(self):
        print("WebSocket connection opened.")
        await self.subscribe_to_tickers()

    async def subscribe_to_tickers(self):
        for ticker in self._tickers:
            subscription_message = {
                "id": self.message_id,
                "cmd": "subscribe",
                "params": {
                    "channels": ["ticker"],
                    "market_ticker": ticker,
                },
            }
            await self.ws.send(json.dumps(subscription_message))
            self.message_id += 1
            print(f"Subscribed to ticker: {ticker}")

    async def handler(self):
        try:
            async for message in self.ws:
                await self.on_message(message)
        except websockets.ConnectionClosed as e:
            await self.on_close(e.code, e.reason)
        except Exception as e:
            await self.on_error(e)

    async def on_message(self, message):
        data = json.loads(message)
        if self._message_handler is not None:
            await self._message_handler(data)
        else:
            print("Received message:", data)

    async def on_error(self, error):
        print("WebSocket error:", error)

    async def on_close(self, close_status_code, close_msg):
        print(
            "WebSocket connection closed with code:",
            close_status_code,
            "and message:",
            close_msg,
        )

    async def subscribe_orderbook_for_ticker(self, ticker: str, also_trades: bool = True):
        channels = [
            {"name": "order_book_delta", "market_ticker": ticker},
        ]
        if also_trades:
            channels.append({"name": "trade", "market_ticker": ticker})

        msg = {
            "id": self.message_id,
            "cmd": "subscribe",
            "params": {"channels": channels},
        }
        await self.ws.send(json.dumps(msg))
        self.message_id += 1
        print(
            f"Subscribed to order_book_delta{' + trade' if also_trades else ''} for {ticker}"
        )
